...
var RequestHandler = /** @class */ (function (_super) {
    ...
    function RequestHandler(config, logger) {
        ...
        var that = _this;
        if (tryAuthenticate) {
            _this.authClient.getToken(function (err) {
                if (err) {
                    that._logger.error([
            ...
            _this.request({
                uri: 'events:report',
                qs: RequestHandler.manufactureQueryString(_this._config.getKey()),
                method: 'POST',
                json: {},
     ...
     * @static
     */
    RequestHandler.manufactureQueryString = function (key) {
        if (isString(key)) {
            return { key: key };
        }
        return null;
