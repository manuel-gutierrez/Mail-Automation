...
var build_stack_trace_1 = require("./build-stack-trace");
/**
...
function populateErrorMessage(ob, em) {
    if (ob === null || ob === undefined) {
        em.setMessage(build_stack_trace_1.buildStackTrace('' + ob));
    }
    else if (ob.stack) {
        ...
        em.setMessage(build_stack_trace_1.buildStackTrace(ob.toString()));
    }
    return em;
}
exports.populateErrorMessage = populateErrorMessage;
/**
...
function populateFromError(err, errorMessage) {
    ...
    if (has(err, 'serviceContext') && isObject(err.serviceContext)) {
        errorMessage.setServiceContext(err.serviceContext.service, err.serviceContext.version);
    }
}
/**
...
function populateFromObject(ob, errorMessage) {
    if (has(ob, 'message')) {
        ...
        errorMessage.setMessage(build_stack_trace_1.buildStackTrace('' + ob));
    }
    if (has(ob, 'user')) {
    ...
    if (has(ob, 'serviceContext') && isObject(ob.serviceContext)) {
        errorMessage.setServiceContext(ob.serviceContext.service, ob.serviceContext.version);
    }
}
//# sourceMappingURL=populate-error-message.js.map